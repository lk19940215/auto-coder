          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {examples.map((example) => (
              <div key={example.id} className="card">
                ...
                <p className="text-sm text-[var(--text-400)] mb-4">✓ {example.result}</p>
                {example.repo && (
                  <a
                    href={example.repo}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-4 py-2 bg-[var(--bg-200)] hover:bg-[var(--primary-600)] rounded-lg text-[var(--text-200)] hover:text-white transition-colors duration-200"
                  >
