...
import { API_KEY, BASE_URL, DEFAULT_MODEL, MAX_TOKENS, DEBUG } from '../config.mjs';
...
define(
  ...
  async ({ description, prompt }) => {
    const tools = SUB_TOOLS
      .filter(n => registry[n])
      .map(n => registry[n].schema);

    const executor = async (name, input) => {
      const tool = registry[name];
      if (!tool || !SUB_TOOLS.includes(name)) return `SubAgent 不允许使用工具: ${name}`;
    ...
    if (trace.stopReason === 'error') return `SubAgent 执行失败: ${trace.error}`;

    const toolNames = [...new Set(trace.toolCalls.map(t => t.name))].join(', ');
    const summary = trace.toolCalls.length > 0
