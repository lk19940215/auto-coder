...
> 1. 首次运行配置脚本（只需一次）：
>    ```powershell
>    .\claude-auto-loop\scripts\setup-windows.ps1
>    ```
...
bash claude-auto-loop/run.sh "你的需求"
  ...
  ┌──────────────────────────────────────────────┐
  │ 1. 项目扫描（首次自动执行）                     │
  │    注入: CLAUDE.md + docs/SCAN_PROTOCOL.md（拼接）  │
  │    Agent 扫描项目文件 → 生成:                   │
...
| `docs/SCAN_PROTOCOL.md` | 扫描专用协议：项目扫描步骤 + `project_profile.json` 格式 + `init.sh` 生成规则（仅在首次扫描时注入） |
...
| `docs/requirements.example.md` | 需求文档模板：复制为 `requirements.md` 填写详细需求 |
...
| `docs/ARCHITECTURE.md` | 工具架构文档：系统概览、Mermaid 时序图、文件职责说明（供 AI Agent 快速理解工具设计） |
| `README.md` | 本文件 |
